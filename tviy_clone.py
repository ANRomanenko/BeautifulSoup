...
def save_doc(items, path):
    with open(path, 'w', encoding='cp1251', newline='') as file:
        ...
        writer.writerow(['Название продукта', 'Ссылка на продукт', 'Код', 'Цена', 'Ссылка на фото'])
        for item in items:
            writer.writerow([item['title'], item['product_link'], item['product-model'], item['price'], item['link_image']])


def parser():
    PAGINATION = input('Введите число страниц для парсинка: ')
    ...
    if html.status_code == 200:
        ...
        for page in range(1, PAGINATION + 1):
            ...
            save_doc(cards, CSV)
        print()
        print('Парсинг завершён! Спарсено:', len(cards), 'карточек товара!')
    else:
        print('Error')
